...
class ThreadsOption : public BuildParameterOption {
  public:
    ThreadsOption()
        : BuildParameterOption("t", "threads", true, INTEGER, ADVANCED) {
    }

    void process(const std::string& arg,
                 BuildParameters& params) const override {
        try {
            int nThreads = std::stoi(arg);
            if (nThreads < 0) {
                logger.logWarning(
                    "Number of threads should be a non-negative integer." +
                    ignoreMessage());
                nThreads = params.nThreads; // reset to default
            }
            params.nThreads = nThreads;
        } catch (...) {
            logger.logWarning("Number of threads should be an integer." +
                              ignoreMessage());
        }
    }

    std::string getDescription() const override {
        return "Number of OpenMP threads to use for suffix-array construction. "
               "0 means all available threads (the default).";
    }
};
