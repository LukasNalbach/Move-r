...
typedef struct {
  ...
  long id;         // rank of the range: ranges are numbered 0,1,... in bwt order
} da_range;
...
typedef struct {
  ...
  int sampledSA;             // START_RUN|END_RUN mask: which sampled SA values are required
  FILE *ssafile, *esafile;   // files for the sampled SA values (start/end of the bwt runs)
  pthread_mutex_t commit_m;  // mutex/condition variable used to write the sampled SA
  pthread_cond_t commit_cv;  // values to file in the order of the corresponding ranges
  long next_commit;          // id of the next range whose samples must be written to file
  int last_bwt;              // last bwt char of the last committed range (Dollar: none yet)
  uint64_t last_sa;          // SA value of that char (only maintained if END_RUN)
...
// ----- computation of the run-sampled SA (options -s/-e) ------------------
// The BWT is generated in ranges which are processed by different threads, so a
// thread cannot tell whether the first char of its range starts a new run: this
// is established later comparing it with the last char of the previous range
// (see ssa_commit). Each thread therefore stores the (pos,SA[pos]) pairs of the
// runs starting/ending strictly inside its range in a local buffer, together
// with the first/last char of the range and their SA values.

// append the pair (pos,sa) to a buffer of samples using SABYTES bytes per value:
// same format as the .ssa/.esa files written by the single thread version
static void append_sample(vector<uint8_t> &buf, uint64_t pos, uint64_t sa)
{
  size_t s = buf.size();
  buf.resize(s+2*SABYTES);
  memcpy(buf.data()+s,&pos,SABYTES);           // low SABYTES bytes, as fwrite(&pos,SABYTES,1,f)
  memcpy(buf.data()+s+SABYTES,&sa,SABYTES);
}

// write a single (pos,sa) pair to a .ssa/.esa file
static void write_sample(FILE *f, uint64_t pos, uint64_t sa)
{
  if(fwrite(&pos,SABYTES,1,f)!=1) die("sampled SA write error (pos)");
  if(fwrite(&sa,SABYTES,1,f)!=1) die("sampled SA write error (sa)");
}

// state of the run-sampled SA computation for a single range of the final BWT
struct ssa_range_state {
  int mode;                 // START_RUN|END_RUN mask
  int first_bwt;            // first bwt char of the range (-1 if the range is still empty)
  uint64_t first_sa;        // SA value of the first bwt char of the range
  int last_bwt;             // last bwt char written so far (-1 if the range is still empty)
  uint64_t last_sa;         // SA value of the last bwt char written so far (only if END_RUN)
  vector<uint8_t> ssa_buf;  // pairs (pos,SA[pos]) of the runs starting inside the range
  vector<uint8_t> esa_buf;  // pairs (pos,SA[pos]) of the runs ending inside the range

  ssa_range_state(int m) : mode(m) {reset();}

  // start the processing of a new range
  void reset() {
    first_bwt = last_bwt = -1;
    first_sa = last_sa = UINT64_MAX;
    ssa_buf.clear(); esa_buf.clear();
  }

  // the SA value of the next bwt char is needed if that char can start a new run
  // (always the case for the first char of the range, whose predecessor is unknown)
  // or if we are sampling the end of the runs, since any char can end a run
  bool sa_needed(int bwtnext) const {
    return (mode & END_RUN) || last_bwt<0 || bwtnext!=last_bwt;
  }

  // account for the bwt char bwtnext at position pos, whose SA value is sa
  // (sa is meaningful only if sa_needed(bwtnext) is true)
  void add_char(uint64_t pos, int bwtnext, uint64_t sa) {
    if(last_bwt<0) {              // first char of the range: handled by ssa_commit
      first_bwt = bwtnext; first_sa = sa;
    }
    else if(bwtnext!=last_bwt) {  // a new run starts at pos, the previous one ends at pos-1
      if(mode & START_RUN) append_sample(ssa_buf,pos,sa);
      if(mode & END_RUN) append_sample(esa_buf,pos-1,last_sa);
    }
    last_bwt = bwtnext;
    if(mode & END_RUN) last_sa = sa;
  }
};
...
// write to the bwt buffer all the characters preceding a given suffix, doing a merge
// operation if necessary, and update the run-sampled SA state of the current range
static void write_chars_same_suffix_ssa(vector<uint32_t> &id2merge,  vector<uint8_t> &char2write,
                                    uint32_t *ilist, uint32_t *istart,
                                    uint8_t *local_bwt, long &c, long bwt_start,
                                    long &easy_bwts, long &hard_bwts,
                                    int_t suffixLen, uint8_t *bwsainfo, long n, ssa_range_state &st)
{
  size_t numwords = id2merge.size(); // numwords dictionary words contain the same suffix
  if(numwords==1) {
    // there is a single run of equal chars, so only its first char can start a new run
    uint32_t s = id2merge[0];
    int bwtnext = char2write[0];
    long num = istart[s+1]-istart[s];
    uint64_t sa = UINT64_MAX;
    if(st.sa_needed(bwtnext))  // SA value of the first char of the run
      sa = get_myint(bwsainfo,n,ilist[istart[s]]) - suffixLen;
    st.add_char(bwt_start+c,bwtnext,sa);
    for(long j=istart[s];j<istart[s+1];j++) // write all bwt chars
      local_bwt[c++] = bwtnext;
    easy_bwts += num;
    if((st.mode & END_RUN) && num>1) // the last char of the group is the one possibly ending a run
      st.last_sa = get_myint(bwsainfo,n,ilist[istart[s+1]-1]) - suffixLen;
  }
  else {  // many words, many chars...
    vector<SeqId> heap; // create heap
    for(size_t i=0; i<numwords; i++) {
    ...
    while(heap.size()>0) {
      ...
      uint64_t sa = UINT64_MAX;
      if(st.sa_needed(bwtnext))
        sa = get_myint(bwsainfo,n,*(s.bwtpos)) - suffixLen;
      st.add_char(bwt_start+c,bwtnext,sa);
      local_bwt[c++] = bwtnext;                 // write BWT value to bwt buffer
      hard_bwts += 1;
      // remove top
      pop_heap(heap.begin(),heap.end());
      heap.pop_back();
...
// write the sampled SA values of a single range to the .ssa/.esa files. Ranges are
// committed in the order of their id, so that: i) the samples are written to file
// in increasing bwt position order, as in the single thread version, and ii) the
// first char of the range can be compared with the last char of the previous range
// to establish whether a bwt run starts at the beginning of the range
static void ssa_commit(thread_data *d, da_range &r, ssa_range_state &st)
{
  xpthread_mutex_lock(&d->commit_m,__LINE__,__FILE__);
  while(d->next_commit != r.id)  // wait for all the previous ranges to be committed
    xpthread_cond_wait(&d->commit_cv,&d->commit_m,__LINE__,__FILE__);
  if(r.count>0) {
    assert(st.first_bwt>=0);
    // d->last_bwt is initialized to Dollar, which is not a valid bwt char, so the
    // very first bwt char always starts a run, as in the single thread version
    if(st.first_bwt != d->last_bwt) {   // a new run starts at the beginning of the range
      uint64_t pos = r.bwt_start;
      if(d->sampledSA & START_RUN)
        write_sample(d->ssafile,pos,st.first_sa);
      if((d->sampledSA & END_RUN) && pos>0) // the previous run ends at pos-1 (if any)
        write_sample(d->esafile,pos-1,d->last_sa);
    }
    // write the samples of the runs starting/ending inside the range
    if((d->sampledSA & START_RUN) && st.ssa_buf.size()>0)
      if(fwrite(st.ssa_buf.data(),1,st.ssa_buf.size(),d->ssafile)!=st.ssa_buf.size())
        die("sampled SA write error (ssa range)");
    if((d->sampledSA & END_RUN) && st.esa_buf.size()>0)
      if(fwrite(st.esa_buf.data(),1,st.esa_buf.size(),d->esafile)!=st.esa_buf.size())
        die("sampled SA write error (esa range)");
    // save the last char of the range (and its SA value) for the next range
    d->last_bwt = st.last_bwt;
    if(d->sampledSA & END_RUN) d->last_sa = st.last_sa;
  }
  d->next_commit++;   // the next range can now be committed
  xpthread_cond_broadcast(&d->commit_cv,__LINE__,__FILE__);
  xpthread_mutex_unlock(&d->commit_m,__LINE__,__FILE__);
}
...
static void *merge_body(void *v)
{
  ...
  ssa_range_state st(d->sampledSA);  // run-sampled SA state of the current range
  // main loop
  while(true) {
    // --- get starting position from buffer 
    xsem_wait(&d->data_items,__LINE__,__FILE__);
    ...
    if(d->sampledSA) st.reset(); // if we need the sampled sa, start a new range
    for(c=0, i = r.start; i<r.end; i=next){
      ...
      if(d->suflen[i]==d->wlen[seqid]) {
        ...
        for(long j=d->istart[seqid];j<d->istart[seqid+1];j++) {
          ...
          if(d->SA) {
            ...
            uint64_t sa = UINT64_MAX;
            if(st.sa_needed(nextbwt)) {
              if(seqid>0) sa = get_myint(d->bwsainfo,d->psize,d->ilist[j]) - d->suflen[i];
              // the first word in the parse is the 2nd lex smaller and starts with Dollar:
              // it generates the very first bwt char, whose SA value is the length of the text
              else {assert(j==1); sa = get_myint(d->bwsainfo,d->psize,0) - d->w;}
            }
            st.add_char(r.bwt_start+c,nextbwt,sa);
          }
          local_bwt[c++] = nextbwt;
          easy_bwts++;
        }
      ...
      if(d->SA)
        ...
        write_chars_same_suffix_ssa(id2merge, char2write, d->ilist,d->istart,local_bwt,c,r.bwt_start,easy_bwts,hard_bwts,
        d->suflen[i],d->bwsainfo,d->psize,st);
      else
    ...
    // if requested write the sampled SA values of this range to the .ssa/.esa files
    // (this must be done in range order, see ssa_commit)
    if(d->sampledSA) ssa_commit(d,r,st);
  }
...
{  
  ...
  td.psize = psize;
  // fields used for the computation of the run-sampled SA
  td.sampledSA = arg.sampledSA;         // sampledSA==0 means sampled SA not requested
  td.ssafile = (arg.sampledSA & START_RUN) ? open_aux_file(arg.basename,EXTSSA,"wb") : NULL;
  td.esafile = (arg.sampledSA & END_RUN) ? open_aux_file(arg.basename,EXTESA,"wb") : NULL;
  xpthread_mutex_init(&td.commit_m,NULL,__LINE__,__FILE__);
  xpthread_cond_init(&td.commit_cv,NULL,__LINE__,__FILE__);
  td.next_commit = 0;
  td.last_bwt = Dollar;                 // this is certainly not a bwt char
  td.last_sa = UINT64_MAX;              // this is an invalid SA entry
  ...
  int pindex=0; da_range r = {0,0,0,0,0};
  for(long i=0; i< dasize; i=next ) {
    ...
    if(entries >= Min_bwt_range) {
      ...
      written += entries; entries=0; r.id++;
    }
    next = i+1;  // prepare for next iteration  
    // discard if it is a small suffix 
  ...
  for(int i=0;i<arg.th;i++)
    xpthread_join(t[i],NULL,__LINE__,__FILE__);
  assert(td.next_commit == (arg.sampledSA ? r.id+1 : 0)); // all ranges have been committed

  // write the last sampled SA pair: the last bwt run ends at the last bwt position
  if(arg.sampledSA & END_RUN) {
    uint64_t pos = td.easy_bwts+td.hard_bwts-1;
    write_sample(td.esafile,pos,td.last_sa);
  }
  ...
  xpthread_cond_destroy(&td.commit_cv,__LINE__,__FILE__);
  xpthread_mutex_destroy(&td.commit_m,__LINE__,__FILE__);
  ...
  if(arg.sampledSA & START_RUN) fclose(td.ssafile);
  if(arg.sampledSA & END_RUN) fclose(td.esafile);
  if(arg.SA or arg.sampledSA!=0) free(bwsainfo); // free sa info
}
